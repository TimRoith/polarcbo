...
for repulsion_scale in repulsion_scales:
    for kappa in kappas:
        ...
        elif algorithm == 'MultiMeanCBS':
            opt = pdyn.CCBS(x, conf.V, beta=conf.beta, tau=conf.tau, mode="sampling",\
                            kernel=kcbo.kernels.Gaussian_kernel(kappa=conf.kappa),\
                            num_means = 2, repulsion_scale = repulsion_scale)              
        else:
        ...
        for i in range(conf.T):
            ...
            if i%20 == 0:
                ...
                if conf.save2disk is True and i in snapshots:
                    filename = cur_path + \
                        "\\visualizations\\MixOfGaussians\\" + algorithm + "-" + case \
                            + "-i-" +str(i) + "-kappa-" + str(conf.kappa) \
                            + "-rep-" + str(repulsion_scale) + ".pdf"
