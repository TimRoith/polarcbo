...
opt = op.KernelCBS(x, conf.V, beta=conf.beta, tau=conf.tau, mode="sampling",\
...
for i in range(conf.T):
    ...
    if i%100 == 0:
        ...
        if conf.save2disk is True and i in snapshots:
            fig.savefig(cur_path+"\\visualizations\\Gaussian\\Gaussian-i-" + str(i) + "-kappa-" + str(conf.kappa) + ".pdf",bbox_inches="tight")
