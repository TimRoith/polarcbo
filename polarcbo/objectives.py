...
class Bimodal():
    ...
    def __call__(self, x):
        ...
        # ret = -np.log(np.exp(-((x[...,0]-1)**2 + (x[...,1]-3)**2/0.2)) \
        #               + 0.5*np.exp( -(x[...,0]+1.2)**2/8 - (x[...,1]+2.5)**2/0.5 ))
        
        # ret = -np.log( 0.5*np.exp( -(x[...,0]-1)**2/8 - (x[...,1]-1.5)**2/0.5 ) \
        #               +0.5*np.exp( -(x[...,0]-1)**2/8 - (x[...,1]+1.7)**2/0.5 ))
        return ret
